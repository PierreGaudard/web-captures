...
RUBRIQUES = ["Réglementation et permis", "Équipement", "Entretien et mécanique",
             "Sécurité et conduite", "Achat et budget", "Voyage et balade"]
...
def construire_hub(cartes, lecture):
    ...
    def meta(c, avec_auteur=True):
        mn = lecture.get(c["url"])
        bout = f"{c['date']}"
        if mn:
            bout += f" · {mn} min de lecture"
        if avec_auteur:
            bout = "Par <b>Prénom Nom</b> · " + bout
        return bout

    html_rail = "".join(f"""
    <a class="mag-mini" href="{c['url']}">
      <div class="vig">{c['img']}</div>
      <div><h3>{c['titre']}</h3><p class="meta">{meta(c, False)}</p></div>
    </a>""" for c in rail)

    html_grille = "".join(f"""
    <a class="mag-card" href="{c['url']}"{crit(5, "Carte d'article",
        "Image en 16/10 recadrée, catégorie en surtitre vert, titre en casse normale, "
        "auteur, date et temps de lecture réel. Aujourd'hui : titre tout en capitales, "
        "extrait tronqué, aucun auteur, et les 20 vignettes du hub ont un alt vide.") if i == 0 else ""}>
      <div class="vig">{c['img']}</div>
      <p class="cat">{c['cat'].upper()}</p>
      <h3>{c['titre']}</h3>
...
{bandeau("hub")}
<div class="wrap">
  <p class="mag-fil"><a href="https://www.amv.fr/">AMV</a><span>›</span>AMV Le Mag<span>›</span>Moto</p>
</div>

<div class="wrap mag-tete"{crit(2, "Titre et chapeau de rubrique",
    "Le hub affiche aujourd'hui « ASSURANCE MOTO », le H1 exact de la landing page "
    "/assurance/moto/, et aucun texte. Deux pages du site se disputent la même requête, et "
    "les moteurs n'ont qu'une liste de titres à lire.")}>
  <h1>Conseils, réglementation et entretien pour motards</h1>
  <p class="chapo">Tout ce qu'il faut savoir avant de prendre la route&nbsp;: <strong>le permis et la
  réglementation</strong>, l'équipement, l'entretien de la machine, la sécurité et les budgets à
  prévoir. Chaque article est rédigé puis relu par des conseillers spécialistes du deux-roues, et
  daté de sa dernière mise à jour.</p>
...
<div class="wrap">
  <div class="mag-hero"{crit(4, "Hero éditorial",
    "Une entrée de rubrique qui hiérarchise, au lieu d'une grille où le dernier publié se "
    "confond avec les 19 autres. Structure reprise de BikeSocial, la marque éditoriale de "
    "l'assureur moto britannique Bennetts.")}>
    <a class="mag-une" href="{une['url']}">
      {une['img']}
      <div class="txt">
        <span class="lbl">À LA UNE</span>
        <h2>{une['titre']}</h2>
        <p class="meta">Par <b>Prénom Nom</b> · {une['date']}{f' · {mn_une} min de lecture' if mn_une else ''}</p>
        <span class="lire">Lire l'article <i class="fas fa-chevron-right"></i></span>
      </div>
    </a>
    <div class="mag-rail">{html_rail}</div>
  </div>
</div>
...
{bandeau("art")}
<div class="art-hero">
  <div class="visuel"><img src="/{HERO_REL}" alt="Motard débutant à côté de sa première moto"
       width="1600" height="900" fetchpriority="high" /></div>
  <div class="wrap">
    <p class="mag-fil" style="color:rgba(255,255,255,.6)">
      <a href="https://www.amv.fr/" style="color:rgba(255,255,255,.6)">AMV</a><span>›</span>
      <a href="{HUB_URL}" style="color:rgba(255,255,255,.6)">Le Mag · Moto</a><span>›</span>Budget</p>
    <span class="sur">ACHAT ET BUDGET</span>
    <h1{crit(2, "Titre et chapeau",
        "H1 unique au format question, en casse normale, suivi d'un chapeau qui pose le sujet. "
        "Le blog met aujourd'hui tous ses H1 en capitales, sans chapeau distinct.")}>Quel budget prévoir pour débuter la moto&nbsp;?</h1>
    <p class="chapo">Permis, équipement, machine, entretien, assurance&nbsp;: le panorama complet des
    frais à anticiper, poste par poste, avec les fourchettes de prix réelles.</p>
    <div class="art-signature"{crit(3, "Signature",
        ...
        "premier écran. C'est le signal E-E-A-T que Google et les moteurs génératifs cherchent. "
        "Le blog n'affiche aujourd'hui qu'une date de publication, parfois vieille de dix ans, "
        "et aucun auteur.")}>
      ...
      <span class="sep">·</span><span>Mis à jour le <b>10 septembre 2026</b></span>
...
<div class="wrap art-wrap">
  <aside class="art-toc"{crit(6, "Sommaire ancré et collant",
    "Il suit la lecture et surligne la section en cours. Navigation interne, ancres nommées, "
    "et surface supplémentaire pour les liens de site dans la SERP. Aucun article du blog n'en "
    "a aujourd'hui.")}>
  ...
  <article class="art-body">
    <div class="art-bref"{crit(4, "Encart « En bref »",
        ...
        "dans notre grille.")}>
      <p class="t">EN BREF</p>
      <p class="cle">Il faut compter entre 5&nbsp;000 et 6&nbsp;000{E} pour un projet moto équilibré,
      en incluant le permis, l'équipement complet, une première moto d'occasion et l'assurance.</p>
      <ul>
        ...
        <li><strong>Première moto d'occasion</strong>&nbsp;: de 2&nbsp;000 à 4&nbsp;000{E} en 125cc,
          de 5&nbsp;000 à 6&nbsp;000{E} en permis A2</li>
    ...
    assurance, entretien&nbsp;: voici les frais à anticiper pour établir un budget clair et adapté à
    vos premiers tours de roue.</p>
    ...
    <table{crit(5, "Tableau de synthèse",
        ...
        "le plus repris tel quel par ChatGPT, Perplexity et les AI Overviews.")}>
    ...
    <p>Le <a href="{L_A2}"{crit(9, "Maillage interne en silo",
        ...
        "lien vers les pages d'assurance.")}>permis A2</a> est la première étape vers le permis A, qui
    prend la forme d'une formation complémentaire de 7 heures.</p>
    ...
    <div class="art-source"{crit(7, "Encart sourcé",
      ...
      <p>Dès la première formule, casque jusqu'à 250 euros, gants jusqu'à 70 euros et gilet airbag
      jusqu'à 500 euros. En souscrivant l'Option Plus, la couverture est étendue à l'ensemble de
      votre équipement moto ainsi qu'aux accessoires hors-série et top case, dans la limite de
      5&nbsp;000 euros.</p>
    ...
    <p><strong>L'occasion est souvent envisagée comme point de départ</strong>, avec des prix très
    ...
    <p><strong>Le coût annuel moyen se situe entre 200 et 400{E} pour une petite cylindrée</strong>,
    50cc ou 125cc, et peut atteindre 1&nbsp;000 voire 1&nbsp;500{E} sur une machine plus puissante.</p>
    <ul>
    ...
    cotisation</strong>&nbsp;: le modèle et la cylindrée, la date de mise en circulation et d'achat,
    le lieu de stationnement, la sinistralité déclarée et le niveau de couverture retenu.</p>
    ...
    Vol / Incendie et la formule <strong>Dommages collision</strong>, qui couvre les accidents avec
    un tiers identifié tout en maîtrisant son budget. Si vous venez d'obtenir votre permis, notre
    page dédiée à l'<a href="{L_JEUNE}">assurance moto jeune conducteur</a> détaille les points à
    connaître.</p>
    ...
    <div class="art-cta"{crit(8, "CTA contextualisé",
        ...
        "et pointant directement la landing page produit.")}>
      <div class="t">
    ...
    4&nbsp;000{E} en permis AM</strong>, <strong>3&nbsp;000 et 6&nbsp;000{E} en permis A1</strong> et
    <strong>6&nbsp;000 et 8&nbsp;000{E} en permis A2</strong>. <strong>Anticiper 5&nbsp;000 à
    ...
    <div class="art-auteur"{crit(11, "Bloc auteur",
      ...
      <div>
        <h3>Prénom Nom</h3>
        <p>Conseiller moto chez AMV depuis X ans. Deux à trois lignes de biographie&nbsp;: parcours,
        spécialité, éventuelle pratique de la moto. Élément à fournir par AMV pour chaque univers,
        avec une photo et un lien de profil.</p>
      </div>
    </div>
